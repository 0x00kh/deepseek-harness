...
Client export 挂载来自 [`@deepseek-ai/dsh-experimental-agent-team/remote`](../agent-team/README.zh.md) 的生成的 `ctx.remote.agentTeams` contribution，然后通过 Cordis effect 注册 locale dictionary 与一个 conversation-header slot。对插件 fiber 执行 dispose（资源释放）会移除这两项 registration。
...
- [Agent Teams Web profile](../agent-team-web-profile/README.zh.md)——挂载本 Client 插件的源码 checkout bundle。
...
无，因为该浏览器 projection 与任务控制界面不注册面向模型的输入。
...
- **快照刷新**——panel 会在打开、显式 refresh 与 mutation 后刷新；它没有实时事件订阅或 mailbox timeline。
