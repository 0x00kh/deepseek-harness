---
description: "dsh Web 客户端设置中按作用域分组的只读插件清单标签页：agent（智能体）预设组合在前，全局平面收在折叠分组里，搜索跨两组。"
...
**插件列表**标签页让 Web 用户查看插件，而不改变其配置。它优先展示 agent 预设组合，并让全局清单保持收起，仅在需要时展开。卡片展示启停状态、出处、运行状态、禁用条件与发现失败；由预设提供的全局条目会标明启用它的预设。搜索覆盖两个分组，并指出其他预设中的匹配。标签页支持加载、空结果、无匹配、失败与重试状态，且不暴露传输细节；没有预设 roster 时仍会展示全局清单。
...
每张收起的卡片使用模块短名称作为标题，并以小标签表示启停状态；已启用的条目还会显示彩色根 fiber 状态圆点。展开卡片后会显示声明的条目 id、完整模块标识与状态事实：预设行说明它来自哪个预设、组合存活时的运行状态，以及它携带的禁用条件；被预设提供的全局行说明它由 Agent 预设按会话提供、列出启用它的预设，并提供跳转到预设组的入口。预设名通过共享的 `presetDisplayText` 折叠（`dsh-agent-presets/display`），基于 [`ui-agent-preset`](../ui-agent-preset/README.zh.md) 的字典解析：内置预设走当前语言，用户自建预设保留自己的元数据，因此英文界面不会回显预设文件里的中文名。搜索按模块名称与条目 id 过滤两组。
